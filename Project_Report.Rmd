...
cbp1 <- c("#56B4E9", "#009E73", "#999999", "#E69F00", 
...
*	Rural-Urban Commuting Areas (RUCA) Codes
...
```{r Indep Vars create plots}
# RUCA codes bar plot
p_ruca <- 
  tb1_dat %>% 
  drop_na() %>% 
  ggplot(aes(x = `RUCA Primary Code 2000`)) + 
  geom_bar(color=cbp1[1], fill=cbp1[1] ) + 
  theme_minimal() + 
...
p_adi <- 
  ...
  filter(fu.period != "0 Month") %>% 
  ggplot(aes(x = ADI_NATRANK)) + 
  geom_histogram(binwidth = 5, color=cbp1[1], fill=cbp1[1] ) + 
  theme_minimal() + 
  theme(plot.title = element_text(size=12)) + 
  ggtitle("National ADI Score") + 
  xlab("")

```

```{r Indep Vars arrange plots, fig.width=10, fig.height=4}
grid.arrange(p_ruca, p_adi, 
             ncol = 2, nrow = 1)
```
...
*	RBANS at 3 and 12 months
*	Trails B at 3 and 12 months
*	Katz (ADLs) at 3 and 12 months
*	FAQ (iADLs) at 3 and 12 months
*	SF-36 at 3 and 12 months
    +	PCS
    +	MCS


```{r Dep Vars (outcomes) create plots}
# RBANS boxplot
p_rbans <- 
  tb2_df %>% 
  ggplot(aes(x = fu.period, y = rbans.global.score, fill = fu.period)) + 
  geom_boxplot() + 
  theme_minimal() + 
  scale_fill_manual(values = cbp1) +
    ...
    ggtitle("RBANS Global Score") +
    xlab("Follow-up Cohorts")


# Trails B boxplot
p_trailsB <- 
  tb2_df %>% 
  ggplot(aes(x = fu.period, y = trail.b.tscore, fill = fu.period)) + 
  geom_boxplot() + 
  theme_minimal() + 
  scale_fill_manual(values = cbp1) +
    theme(
...
p_katz <- 
  tb2_df %>% 
  ggplot(aes(x = fu.period, y = adl.totscore, fill = fu.period)) + 
  geom_boxplot() + 
  theme_minimal() + 
  scale_fill_manual(values = cbp1) +
    theme(
...
p_faq <- 
  tb2_df %>% 
  ggplot(aes(x = fu.period, y = faq.totscore, fill = fu.period)) + 
  geom_boxplot() + 
  theme_minimal() + 
  scale_fill_manual(values = cbp1) +
    theme(
...
p_sf36_pcs <- 
  tb2_df %>% 
  ggplot(aes(x = fu.period, y = sf36.pcs, fill = fu.period)) + 
  geom_boxplot() + 
  theme_minimal() + 
  scale_fill_manual(values = cbp1) +
    theme(
    ...
    ggtitle("SF-36 Physical Component Score") +
    xlab("Follow-up Cohorts")
...
p_sf36_mcs <- 
  tb2_df %>% 
  ggplot(aes(x = fu.period, y = sf36.mcs, fill = fu.period)) + 
  geom_boxplot() + 
  theme_minimal() + 
  scale_fill_manual(values = cbp1) +
    theme(
    ...
    ggtitle("SF-36 Mental Component Score") +
    xlab("Follow-up Cohorts")


```

```{r Dep Vars (outcomes) arrange plots, fig.width=12, fig.height=9}
grid.arrange(p_rbans, p_trailsB, p_katz, p_faq, p_sf36_pcs, p_sf36_pcs, 
             ncol = 3, nrow = 2)
```
...
*	IQCODE (for cognitive outcomes models)
*	Baseline Katz (for disability models)
*	Baseline FAQ (for disability models)
*	Mean Modified Daily SOFA
*	Days of Delirium/Coma
*	Days of Mechanical Ventilation
*	Education
*	Sex
*	Hospital Type (VA versus Civilian)
